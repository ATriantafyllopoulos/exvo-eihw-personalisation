...
class InstFreqNorm(nn.Module):
    def __init__(self, lambd, eps=1e-5, freq_dim=1):
        super().__init__()
        self.lambd = lambd
        self.eps = eps
        self.freq_dim = freq_dim
    def forward(self, x):
        mean = x.mean(self.freq_dim, keepdim=True)
        std = x.std(self.freq_dim, keepdim=True)
        norm_x = (x - mean) / torch.sqrt(std + self.eps)
        return self.lambd * x + norm_x
...
class SincNet(nn.Module):
    ...
    def __init__(self,options):
       ...
       for i in range(self.N_cnn_lay):
         ...
         if self.cnn_use_batchnorm[i] == 'InstFreqNorm':
           self.bn.append(InstFreqNorm(0.1))
         else:
